# @description:A股上市公司情况信息保存到数据库表
# @Author: 周健平
# @company: 山东大学
# @Time: 2020/7/11 20:41
import time
...
def generate_mysql( ):
    ...
    # listed_company是要在db1数据库中建立的表，用于存放数据

    cursor.execute( sql )
    conn.close( )


def write_to_sql( tbl , db='labsql' ):
    engine = create_engine( 'mysql+pymysql://root:sdu.292653@localhost:3306/{0}?charset=utf8'.format( db ) )
    try:
        # df = pd.read_csv(df)
